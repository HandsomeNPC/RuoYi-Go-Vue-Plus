...
//
// CheckOrigin 恒放行：跨域策略统一由 pkg/middleware 的 CORS 段决定
// （对齐 Java allowedOrigins 默认 '*'）。注意 CORS 中间件管不到 WebSocket
// 握手——浏览器对 ws:// 不发预检，故这里必须自己表态，否则 gorilla 默认
// 只接受同源请求，前端本地开发（vite 端口与后端不同源）会被直接拒掉。
...
// 拿不到登录态就没法投递（对齐 Java 握手拦截器先取 LoginUser 的做法）。
...
// CloseHandler 返回主动断开当前连接的端点（对应 Java SseController.close）。
...
func (m *Manager) serveWebSocket(c *gin.Context) {
	...
	if !ok {
		...
		// 拿不到原因（对齐 Java 校验失败即 CloseStatus.BAD_DATA，但这里
		// 还没握手，可以更直接地回 401）。
...
//
// 目前只认心跳。Java 侧会把其它文本当自定义消息回发给该用户（PushTypeEnum.CUSTOM），
...
// isPing 判定是否心跳请求。
//
// 同时认裸字符串 "ping" 与 {"type":"ping"} 两种形态：Java 侧比对的是裸串，
...
// 一返回，net/http 就会结束响应体并关闭连接。故这里不能像 Java 的 SseEmitter
// 那样"注册完就返回"，必须自己守在循环里。
func (m *Manager) serveSSE(c *gin.Context) {
	...
	// 先回一条注释确认连接建立（对齐 Java comment("connected")）：
	// 前端据此知道流已就绪，且能立刻探明代理有没有缓冲住响应。
	if err := sc.writeRaw([]byte(sseComment("connected"))); err != nil {
		return
	}

	// 超时上限对齐 Java sse-timeout：到点主动收流，由前端 EventSource 自动重连。
...
// prepareSSEHeaders 设置 SSE 响应头（对齐 Java prepareSseResponse）。
