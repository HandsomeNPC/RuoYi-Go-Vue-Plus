...
type Tree[K comparable] struct {
	...
	Name   string
	Weight int
	// Extra 动态扩展键，序列化时平铺到与 id/label 同层。
...
//
// 与 Build/BuildMultiRoot 不同，本函数**不排序**——只做分组挂接，
...
//
// 只做分组挂接、不递归下钻：父节点不在 nodes 里的子树不会冒到顶层。
