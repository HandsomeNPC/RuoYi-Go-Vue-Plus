...
// smsAuthStrategy 短信验证码认证策略（对应 Java SmsAuthStrategy）。
...
//
// 对照 Java SmsAuthStrategy.validateSmsCode：Java 在 checkLogin 的 supplier 内被调用，
// 验证码为空 throw CaptchaExpireException（越过计数分支、不计重试），否则返回 code.equals(smsCode)
// 交由 supplier 取反作失败计数。Go 无异常，故返回 (bool, error)：过期返回 error 由调用方直接返回
// （不进 CheckLogin），匹配返回 bool 供 CheckLogin 闭包作计数判定。
//
// 与图形验证码不同，短信码只读不删：Java 侧亦只读，码自有 TTL 自然过期。
