...
type SysUserExportVo struct {
	...
	// DeptID 导出时回填成部门名称显示（Go 侧由 service 直接写 DeptName 列）。
	// 这里 DeptName 独立成列承接部门名，DeptID 本身不单列导出。
	...
	// Gender 用户性别（0男 1女 2未知），导出按 excelDict 转标签。
	Gender string `json:"gender" excel:"用户性别" excelDict:"0=男,1=女,2=未知"`
	// Status 账号状态（0正常 1停用），导出按 excelDict 转标签。
