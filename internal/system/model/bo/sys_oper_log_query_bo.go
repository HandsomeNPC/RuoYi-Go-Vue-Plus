...
// SysOperLogQueryBo 操作日志列表查询条件（query 参数），对齐 Java buildQueryWrapper。
//
// 与 SysOperLogBo 分开而非复用：查询条件全部可选，SysOperLogBo 是记录落库的写入契约。
// 过滤口径照抄 Java：operIp/title/operName/browser/os 走 like、businessType/status 走 eq、
...
type SysOperLogQueryBo struct {
	...
	// BusinessType 单值业务类型。对齐 Java 的 businessType > 0：0=其他 不参与单值过滤，
