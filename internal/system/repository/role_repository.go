...
// SelectPageList 按条件分页查角色（对应 Java selectPageRoleList）。
func (r *RoleRepository) SelectPageList(ctx context.Context, q bo.SysRoleQueryBo,
	...
	// 仅在调用方未指定排序时兜底（对齐 Java orderByAsc(roleSort, createTime)）。
...
// SelectByIDs 按主键批量查，返回实际命中的行（对应 Java selectByIds，
// 供删除前校验取角色名用）。缺失主键静默跳过，由调用方比对数量。
...
// （对应 Java selectRoleByIds：ids 为空即不加 IN 条件，退化成查全部启用角色）。
func (r *RoleRepository) SelectNormalByIDs(ctx context.Context,
	...
	// Java 未指定排序，这里固定按 role_sort 升序：下拉框选项顺序不该随 MySQL 返回顺序漂移。
...
// （对齐 Java checkRoleNameUnique 的 neIfPresent，供修改场景排除自身）。
...
// （对齐 Java checkRoleKeyUnique 的 neIfPresent，供修改场景排除自身）。
...
// CountByRoleIDs 按主键集合统计存在的角色数（对应 Java selectRoleCount）。
//
// Java 此方法带 @DataPermission，在非超管账号下注入部门/角色数据隔离条件，
// 使"我能看到多少个这些角色"与传入数量不等时抛"没有权限访问部分角色数据"。
// Go 侧数据权限尚未落地，此处只按主键计数——超管在 service 层短路，非超管
...
// DeleteRoleMenuByRoleIDs 清理给定角色的菜单关联（对应 Java updateRolePermission
// 与 deleteRoleById 的 roleMenu 清理分支）。sys_role_menu 无 del_flag，物理删除。
...
// InsertRoleMenuBatch 批量写入角色-菜单关联（对应 Java insertRoleMenu 的批量形态）。
...
// DeleteRoleDeptByRoleIDs 清理给定角色的部门（数据权限）关联（对应 Java updateRolePermission
// 与 deleteRoleById 的 roleDept 清理分支）。sys_role_dept 无 del_flag，物理删除。
...
// InsertRoleDeptBatch 批量写入角色-部门（数据权限）关联（对应 Java insertRoleDept 的批量形态）。
...
// SelectDeptIDsByRoleID 按角色取其勾选的部门主键（对应 Java selectDeptListByRoleId）。
//
...
// SelectUserIDsByRoleID 取已分配该角色的用户ID集合（对应 Java SysUserRoleMapper#selectUserIdsByRoleId）。
...
// CountUserRoleByRoleID 统计已分配该角色的用户数（对应 Java countUserRoleByRoleId）。
// 角色被引用即不可删/不可禁用。
...
// DeleteUserRole 取消单个用户的角色授权（对应 Java deleteAuthUser 的 userRoleMapper 分支）。
...
// DeleteUserRolesByRoleID 批量取消角色下的用户授权（对应 Java deleteAuthUsers）。
...
// InsertUserRoles 批量给角色追加用户授权（对应 Java insertAuthUsers）。
