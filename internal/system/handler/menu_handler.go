...
// List 查询菜单列表。
//
// 不分页：菜单总量有限，前端拿扁平列表后自行构树整树渲染，与 Java 一致。
...
// TreeSelect 获取菜单下拉树列表（对应 Java treeselect）。
...
// RoleMenuTreeSelect 加载对应角色的菜单列表树及其选中节点（对应 Java roleMenuTreeselect）。
//
...
// CascadeRemove 批量级联删除菜单，同时清理角色授权（对应 Java cascade/{menuIds}）。
// 主键串以逗号分隔，与 Java 的 @PathVariable Long[] menuIds 同形。
