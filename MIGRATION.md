...
**`config.Load` 必须早于 `middleware.Register`**（`Auth` 还要求先 `redis.Init`）。 `config.Load` 不返回配置、 失败直接
panic（与 `Get()` 同语义：启动期错误不往运行时带），配置一律经 `config.Get()` 取（含 `main` 自身），不逐层传 `*config.Config`。
